#!/usr/bin/env Rscript
# ============================================================================
# Differential binding with a SELECTABLE normalization mode (CUT&RUN, no spike-in)
# ============================================================================
...
#
#   --mode anchor   Median-of-ratios restricted to a fixed INVARIANT reference
#                   BED (--anchor; default the shipped constitutive-CTCF set),
#                   with iterative trimming of anchors that move between
#                   conditions. Spike-in free. Most principled when the target
#                   has signal at the anchor regions; like `none` it defines the
...
#                   the sample's own transcriptome. Like `none`/`anchor` it defines
#                   the anchor set as invariant, so a true global shift is
#                   undetectable by construction.
#
# Inputs (produced by this workflow):
#   results/consensus[_seacr]/consensus_counts.txt   featureCounts matrix + coords
#   config/samples.csv                               design (sample_id, condition, replicate)
#   ref/constitutive_ctcf_hg38.bed                   mode=anchor default (--anchor)
#
# Usage:
#   Rscript workflow/scripts/diffopen.R --mode anchor \
#       --counts  results/consensus/consensus_counts.txt \
#       --samples config/samples.csv \
#       --anchor  ref/constitutive_ctcf_hg38.bed \
#       --outdir  results/diffopen/macs2/anchor [--ref-label Control]
# ============================================================================
...
anchor_overlap <- function(coords, ctcf_path) bed_overlap(coords, ctcf_path)
...
#' Median-of-ratios restricted to a subset of rows (the anchors).
...
size_factors_anchor <- function(counts, anchor_idx, condition,
...
size_factors_rnastable <- function(counts, coords, promoter_is, tx, de,
  ...
  fit <- size_factors_anchor(counts, idx, condition,
...
main <- function() {
  ...
  if (nlevels(droplevels(des$condition)) < 2)
    stop("need >= 2 conditions in samples.csv 'condition' column for a differential test")
  ...
  if (mode == "none") {
    ...
    if (n_anchors < as.integer(a$`min-anchors`))
      stop(sprintf("only %d anchors (< %s); refusing to normalize on so few",
                   n_anchors, a$`min-anchors`))
    fit <- size_factors_anchor(counts, idx, des$condition,
  ...
  if (!is.null(cls)) {
    ...
    class_lines <- c(class_lines,
      ...
      "",
      "  Compare modes before trusting any single one: `none` and `anchor` should",
  ...
  summ <- c(
    ...
    if (mode == "rnastable") c(
      ...
      sprintf("rnastable anchors         : %d promoter-class over stable TSS -> %d kept after trim",
              n_anchors, n_kept),
      "note                      : like none/anchor, assumes anchors invariant; cannot see a uniform global shift"
    ) else NULL,
