#!/usr/bin/env python3
"""Build a single self-contained interactive HTML QC report for the CUT&RUN
...
def build_data(results_dir, samples):
    ...
    # Peaks + FRiP — MACS2 (drives the per-sample FRiP flag) and SEACR (parallel)
    ps = read_tsv(f"{R}/qc/peak_summary_macs2.tsv") if os.path.exists(
        f"{R}/qc/peak_summary_macs2.tsv") else []
    for row in ps:
        try:
    ...
    ps_seacr = read_tsv(f"{R}/qc/peak_summary_seacr.tsv") if os.path.exists(
        f"{R}/qc/peak_summary_seacr.tsv") else []
    sections["peaks_seacr"] = {"rows": ps_seacr, "flagkey": None, "flagspec": None}
    ...
    UNITS = {
        ...
        "peaks":      ("reads · fragments", "frag"),
        "peaks_seacr":("reads · fragments", "frag"),
